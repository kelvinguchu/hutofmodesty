...
export function UserButton({
  ...
  if (isLoading) {
    return (
      <div
        className={`${paddingStyles} ${isMobile ? "" : "rounded-full"} ${className}`}>
  ...
  return (
    <Link
      ...
      <div className='relative'>
        <Avatar
          className={`w-8 h-8 ${isMobile ? "" : "ring-2 ring-transparent group-hover:ring-purple-200 transition-all duration-200"}`}>
          {profilePhotoUrl && (
            <AvatarImage
              src={profilePhotoUrl}
        ...
        {/* Online status indicator - only show on desktop */}
        {!isMobile && (
